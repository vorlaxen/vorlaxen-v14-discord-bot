...
export async function loadEvents(client: VorlaxenBot): Promise<void> {
  ...
  if (!existsSync(modulesPath)) {
    logger.warn(
      { path: modulesPath },
      '[EventManager] Module root directory not detected.',
    );
    return;
  }
  ...
  for (const moduleName of moduleDirs) {
    ...
    if (existsSync(eventsPath) && statSync(eventsPath).isDirectory()) {
      ...
      logger.info(
        ...
        `[EventManager] Module scan completed: ${moduleName}`,
      );
    }
  }
  ...
  for (const filePath of allEventFiles) {
    try {
      ...
      if (event?.name) {
        ...
        logger.info(
          { source: fileName, mode: event.once ? 'ONCE' : 'ON' },
          `[EventManager] Listener established: ${event.name}`,
        );
      }
    } catch (error) {
      logger.error(
        { source: filePath, err: error },
        '[EventManager] Failed to initialize event listener.',
      );
    }
  }
}
